...
export function sample(site="A") {
 ...
 return {site, mainRouter:"Home router", labRouter:"Experimental router", firewall:"Trusted firewall", firewallPlatform:"openwrt", labCidr:"10.77."+n+".0/24", target:"10.77."+n+".20", pico:"10.77."+n+".30", relayCidr:"10.78."+n+".0/24", managementCidr:"10.79."+n+".0/24", serviceCidr:"10.80."+n+".0/24", homeTransitCidr:"10.81."+n+".0/24", homeCidr:"", relayHost:"relay-"+site.toLowerCase()+".example.invalid", gameHost:"games-"+site.toLowerCase()+".example.invalid"};
}
export const platforms={openwrt:"OpenWrt",pfsense:"pfSense",opnsense:"OPNsense"};
...
export function validate(p) {
 const errors=[];
 if(!Object.hasOwn(platforms,p.firewallPlatform)) errors.push("Choose OpenWrt, pfSense or OPNsense as the firewall platform.");
 ...
 const keys=["labCidr","relayCidr","managementCidr","serviceCidr","homeTransitCidr",...(p.homeCidr?["homeCidr"]:[])];
 const nets=keys.map(k=>network(p[k]));
 keys.forEach((k,i)=>{ if(!nets[i]) errors.push(k+": enter a network ending in .0/24."); });
 nets.forEach((a,i)=>{if(a && !(a[0]===10 || (a[0]===172 && a[1]>=16 && a[1]<=31) || (a[0]===192 && a[1]===168))) errors.push(keys[i]+": use an RFC1918 private subnet.");});
 if(new Set(keys.map(k=>p[k])).size!==keys.length) errors.push("Home, home transit, lab, relay, management and services must use distinct subnets.");
...
export const scenarios = [
 ...
 {id:"edge",title:"Firewall at the edge",label:"DIAGRAM 1 · HOME CUTOVER REQUIRED",description:"A pfSense or OPNsense gateway sits between the ISP and both home and lab. The home router's WAN uses its own filtered transit network; lab targets occupy separate ports or VLANs.",doc:"network/PFSENSE-OPNSENSE.md",requirements:["A supported maintained firewall computer and enough independent interfaces, or a managed VLAN switch. The home connection now depends on this firewall: prepare a rollback before moving cables.","Keep the home router in router mode for this drawing. Its WAN uses the home-transit subnet; its existing LAN stays distinct. This normally creates double NAT for home devices.","A Linux Pi running Docker is a lab target, never the trusted boundary or relay. Keep home and management VLANs off that host. CrowdSec is optional; explicit firewall rules still provide isolation."]},
 {id:"split",title:"Separate ISP branches",label:"DIAGRAM 2 · ISP SUPPORT REQUIRED",description:"An ISP handoff feeds a WAN-only switch, then the home-router WAN and lab-firewall WAN separately. Each router needs its own ISP-authorized connection; this design is conditional on the service contract and handoff.",doc:"network/PFSENSE-OPNSENSE.md",requirements:["Confirm that the ISP permits both router connections, including address allocation, leases, authentication and VLAN requirements. A switch does not create public IP addresses. A routed address block is a different configuration.","Connect only the ISP handoff and the two router WAN ports to this switch. Home LAN, targets and administration must remain behind their respective firewalls.","Both WAN interfaces are exposed to the provider network. Deny lab-initiated traffic to the home router's public endpoint too; CGNAT, IPv6 and provider filtering still matter. Docker containers remain inside the untrusted lab."]},
...
export function rows(id,p) {
 ...
 if(id==="edge" || id==="split") return [["Firewall software",platforms[p.firewallPlatform]],...lab,["Relay subnet / host",p.relayCidr+" / "+base(p.relayCidr)+".2"],["Management subnet",p.managementCidr],["Clean services subnet",p.serviceCidr],["Home LAN",p.homeCidr||"Existing range; record privately"],...(id==="edge"?[["Home-transit subnet",p.homeTransitCidr],["Firewall HOME gateway",base(p.homeTransitCidr)+".1"],["Home router WAN (planned)",base(p.homeTransitCidr)+".2"]]:[["ISP WAN addresses","Separately assigned by ISP; not collected"],["Shared WAN switch","ISP + router WAN ports only"]]),["Remote access",p.relayHost+" via dedicated Linux relay"],["Lab → other zones / internet","Denied; replies to approved sessions only"]];
...
export function command(id,p) {
 ...
 if(id==="edge" || id==="split" || id==="single" || p.firewallPlatform!=="openwrt") return "# "+platforms[p.firewallPlatform]+" / "+scenarios.find(s=>s.id===id).title+"\n# Complete network/ARCHITECTURES.md and the interface/rule worksheet.\n# For BSD platforms and the new placements: network/PFSENSE-OPNSENSE.md.\n# No firewall configuration is generated for this platform/topology.\n# The OpenWrt UCI format cannot be imported into pfSense or OPNsense.\n# Optional validation on the isolated Linux lab laptop only:\n# A successful target request does not prove isolation.\ncurl --max-time 3 http://"+p.pico+":8080/status\n# Token mode requires the private Authorization header.";
...
export function steps(id,p){
 const table={
 edge:[["The ISP reaches the edge firewall","The maintained "+platforms[p.firewallPlatform]+" gateway is now the entry point. Its WAN must match the ISP handoff; unsolicited inbound traffic stays denied.","internet"],["Home has a dedicated branch","The home router's WAN is "+base(p.homeTransitCidr)+".2 on "+p.homeTransitCidr+". Its existing LAN remains behind it, with an additional NAT layer.","home"],["Targets have a different attachment","A Linux Pi or Docker host belongs only to the lab zone "+p.labCidr+". Containers share the host kernel and cannot be the containment boundary.","target"],["The firewall denies new lab sessions","Rules on the lab interface deny home transit, home LAN, management, services, relay and internet destinations. Only replies to permitted incoming sessions return.","firewall"],["Remote access uses a separate relay","The approved VPN and SSH relay stays on "+p.relayCidr+". It may reach "+p.pico+":8080 and "+p.target+":8081 only through specific firewall permissions.","relay"]],
 split:[["Two ISP connections must be permitted","The provider must authorize the home router and the lab firewall separately. Multiple leases, a routed block and a private router uplink are different services.","internet"],["The switch carries only WAN traffic","The unmanaged switch distributes the ISP Ethernet handoff. It provides neither public addresses nor firewall protection, and no LAN device belongs on it.","switch"],["Home remains behind its own router","The home router WAN has its own ISP-assigned address. The lab firewall must also block paths toward this public endpoint, not merely the home private subnet.","home"],["The lab firewall contains its targets","Targets live on "+p.labCidr+" behind "+platforms[p.firewallPlatform]+". Lab-originated forwarding is denied; the shared provider segment must never be bridged into this LAN.","firewall"],["The Pi remains an untrusted host","Docker lab containers share the Linux host kernel. Keep the relay and clean game services in separate zones and hosts; optional VPN access follows the same narrow relay policy.","target"]],
...
const iconPaths={
 ...
 ,switch:'<rect x="2" y="11" width="38" height="24" rx="2"/><path d="M8 20h6v7H8zm10 0h6v7h-6zm10 0h6v7h-6M10 5h22m-4-3 4 3-4 3"/>'
};
...
export function diagram(id,p,active=""){
 ...
 body+=boundary(20,270,310,175,"HOME / SEPARATE TRANSIT")+boundary(770,25,310,185,"UNTRUSTED LAB")+boundary(770,260,310,185,"DEDICATED RELAY / SEPARATE ZONE");
 body+=link(250,110,425,110,"ISP handoff")+link(660,110,795,110,"LAB")+link(480,154,480,225)+link(480,225,175,225,p.homeTransitCidr)+link(175,225,175,320)+link(605,154,605,245)+link(605,245,715,245)+link(715,245,715,360)+link(715,360,795,360,"RELAY");
 body+=node("internet",55,66,"Internet / ISP","WAN connection","globe",active)+node("firewall",425,66,short(p.firewall),platforms[p.firewallPlatform],"firewall",active,235)+node("home",50,320,short(p.mainRouter),"WAN "+base(p.homeTransitCidr)+".2","home",active,250)+node("target",795,66,"Linux Pi / lab targets",p.labCidr,"server",active,260)+node("relay",795,316,"Trusted Linux relay",base(p.relayCidr)+".2","server",active,260);
 body+='<text x="798" y="180" font-size="10">Docker shares the target host kernel.</text><text x="363" y="293" font-size="11">× LAB → HOME / WAN / other zones</text><text x="363" y="328" font-size="10">MGMT and clean SERVICES also need</text><text x="363" y="346" font-size="10">separate ports or configured VLANs.</text><text x="363" y="414" font-size="10">CrowdSec optional · rules enforce isolation</text>';
 }else if(id==="split"){
 body+=boundary(20,275,310,170,"PROTECTED HOME LAN")+boundary(585,275,275,170,"LAB SECURITY BOUNDARY")+boundary(885,275,195,170,"UNTRUSTED LAB");
 body+=link(250,105,410,105,"ISP Ethernet handoff")+link(520,149,520,223)+link(520,223,175,223,"ISP-authorized WAN 1")+link(175,223,175,315)+link(520,223,722,223,"ISP-authorized WAN 2")+link(722,223,722,315)+link(840,359,895,359);
 body+=node("internet",55,61,"Internet / ISP","Two connections required","globe",active)+node("switch",410,61,"WAN-only switch","No LAN devices here","switch",active,220)+node("home",50,315,short(p.mainRouter),"WAN: ISP assigned","home",active,250)+node("firewall",605,315,short(p.firewall),platforms[p.firewallPlatform],"firewall",active,235)+node("target",895,315,"Lab targets",p.labCidr,"server",active,175);
 body+='<text x="670" y="85" font-size="12">Conditional: ISP must allow both routers.</text><text x="670" y="108" font-size="10">An unmanaged switch does not create public IPs.</text><text x="348" y="324" font-size="11">× No LAN bridge</text><text x="349" y="347" font-size="10">Keep both WANs</text><text x="349" y="365" font-size="10">default-deny.</text><text x="605" y="425" font-size="9">MGMT / RELAY / SERVICES: separate internal zones</text>';
 }else if(id==="single"){
...
export function publicFigure(id,site="A") {
 const p=sample(site);if(id==="edge" || id==="split")p.firewallPlatform="pfsense";
 return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1100 505" font-family="Arial,Helvetica,sans-serif">'+diagram(id,p)+'<text x="25" y="490" font-size="10" fill="#555">PUBLIC EXAMPLE · ALL ADDRESSES FICTIONAL · '+esc(scenarios.find(s=>s.id===id).title)+'</text></svg>';
}

export function buildInputs(plan,scenario="tunnel") {
 const errors=validate(plan);
 if(!scenarios.some(s=>s.id===scenario)) errors.push("Choose a documented architecture.");
 ...
 return {
  site:plan.site, firewall_platform:plan.firewallPlatform, architecture:scenario, addresses_confirmed:false, isolated_lab_confirmed:false,
