...
import {esc, sample, validate, scenarios, rows, command, steps, diagram, concepts, icon, publicFigure, buildInputs} from "./model.js";
const $=id=>document.getElementById(id);
const SITE_ROOT=new URL("../",import.meta.url);
const resource=path=>new URL(path,SITE_ROOT).href;
const state={scenario:"offline",site:"A",step:0,plans:{A:sample("A"),B:sample("B"),C:sample("C")},edited:new Set(),private:false};
...
function renderForm(){const p=state.plans[state.site];$("plan-fields").innerHTML=fields.map(([key,label])=>'<label>'+label+'<input name="'+key+'" value="'+esc(p[key])+'" maxlength="'+(["mainRouter","labRouter","firewall"].includes(key)?48:253)+'" autocomplete="off" spellcheck="false"></label>').join("");$("plan-error").textContent="";}
function render(){
 ...
 $("recipe-link").dataset.doc=s.doc;$("connection-command").textContent=command(s.id,p);
...
$("scenario-buttons").addEventListener("click",e=>{const b=e.target.closest("[data-scenario]");if(b){state.scenario=b.dataset.scenario;state.step=0;render();}});
document.querySelectorAll("[data-site]").forEach(b=>b.addEventListener("click",()=>{state.site=b.dataset.site;state.step=0;render();}));
...
$("reset-plan").onclick=()=>{state.plans[state.site]=sample(state.site);state.edited.delete(state.site);render();toast("Fictional example restored.");};
...
$("download-config").onclick=()=>{try{download("config.local.json",JSON.stringify(buildInputs(state.plans[state.site]),null,2)+"\n");toast("Private builder inputs downloaded. Wi-Fi and deployment remain disabled.");}catch(error){toast(error.message);}};
...
const hardware=[
 ...
 ["server","FULL COMPUTER","Linux Pi / PC / appliance","A full Linux Raspberry Pi, suitable PC or VM can run an SSH relay. Game servers need compatible CPU architecture and adequate resources.","ADD: TRUSTED FIREWALL + LINUX RELAY"]
];
...
function markdown(text,path){
 ...
 for(let i=0;i<lines.length;i++){const line=lines[i];
  ...
  const h=line.match(/^(#{1,6})\s+(.+)$/);if(h){close();out+="<h"+h[1].length+">"+inline(h[2],path)+"</h"+h[1].length+">";continue;}
...
document.addEventListener("click",e=>{const link=e.target.closest("[data-doc]");if(link){e.preventDefault();loadDoc(link.dataset.doc);if(!link.closest("#doc-content")&&!link.closest("#doc-nav"))$("library").scrollIntoView();}});
